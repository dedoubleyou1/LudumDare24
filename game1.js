...
window.statTypes = ['health', 'strength', 'agility', 'precision', 'fire', 'water', 'grass'];

var bodyParts = [
		{ name: 'head', elementClass: '.head', elementId: '#headAbility' },
		{ name: 'body', elementClass: '.body', elementId: '#bodyAbility' },
		{ name: 'feet', elementClass: '.tail', elementId: '#feetAbility' }
];
...
window.updateAbilities = function () {
	bodyParts.forEach(function(bodyPart){
		$( bodyPart.elementId ).html( window.assetStats[ window.character[ bodyPart.name ]].ability.name );
	});
};
...
window.startApp = function () {
	...
	$('#randomize').click(function () {
		bodyParts.forEach( function( bodyPart ) {
			window.randAsset( 'character', bodyPart );
		});
	});

	$('#fight').click(function () {
		bodyParts.forEach( function( bodyPart ) {
			window.randAsset( 'enemy', bodyPart );
		});
	...
	var abilityClickHandler = function(bodyPartName) {
		window.assetStats[ window.character[ bodyPartName ]].ability.attack( 'character', 'enemy' );
		window.enemyAttack();
		window.testDamage();
	};
	
	bodyParts.forEach(function(bodyPart) {
		$(bodyPart.elementId).click(function() {
			abilityClickHandler(bodyPart.name);
		});
	});
